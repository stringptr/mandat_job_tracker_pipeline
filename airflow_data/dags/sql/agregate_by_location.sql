INSERT INTO gold.junior_job_data_engineer_by_location SELECT
  ...
  MIN(salary_min) FILTER (
WHERE
  salary_min > 0) AS min_salary,
  MAX(salary_max) FILTER (
WHERE
  salary_max > 0) AS max_salary,
  AVG((salary_min + salary_max) / 2) FILTER (
WHERE
  salary_min > 0
  AND salary_max > 0) AS avg_salary
FROM
  silver.adzuna_raw
WHERE
